    ...
    questions: [
        {
            id: 1,
            type: 'push',
            question: '일상에서 벗어나 강렬한 해방감을 느끼고 싶나요?',
            description: 'Push: 탈출·휴식',
            options: [
                { text: '매우 그렇다', scores: { carnival: 3, tomatina: 3, oktoberfest: 2 } },
                { text: '보통이다', scores: { carnival: 2, tomatina: 2, oktoberfest: 2 } },
                { text: '아니다', scores: { carnival: 0, tomatina: 0, oktoberfest: 1 } }
            ]
        },
        {
            id: 2,
            type: 'pull',
            question: '수만 명의 인파 속에서 에너지를 얻는 편인가요?',
            description: 'Pull: 축제의 열기',
            options: [
                { text: '매우 그렇다', scores: { carnival: 3, tomatina: 2, oktoberfest: 3 } },
                { text: '보통이다', scores: { carnival: 2, tomatina: 2, oktoberfest: 2 } },
                { text: '아니다', scores: { carnival: 0, tomatina: 1, oktoberfest: 0 } }
            ]
        },
        {
            id: 3,
            type: 'pull',
            question: '전통 음식과 음악을 즐기는 것을 좋아하나요?',
            description: 'Pull: 문화·음식',
            options: [
                { text: '매우 그렇다', scores: { carnival: 2, tomatina: 1, oktoberfest: 3 } },
                { text: '보통이다', scores: { carnival: 1, tomatina: 1, oktoberfest: 2 } },
                { text: '아니다', scores: { carnival: 0, tomatina: 1, oktoberfest: 0 } }
            ]
        },
        {
            id: 4,
            type: 'push',
            question: '친구들과 특별한 추억을 만드는 것이 가장 중요한가요?',
            description: 'Push: 사회적 유대',
            options: [
                { text: '매우 그렇다', scores: { carnival: 2, tomatina: 3, oktoberfest: 3 } },
                { text: '보통이다', scores: { carnival: 1, tomatina: 2, oktoberfest: 2 } },
                { text: '아니다', scores: { carnival: 0, tomatina: 0, oktoberfest: 1 } }
            ]
        }
    ],
...
let quizAnswers = [];
let festivalScores = { carnival: 0, tomatina: 0, oktoberfest: 0 };

function startQuiz() {
    currentQuestionIndex = 0;
    quizAnswers = [];
    festivalScores = { carnival: 0, tomatina: 0, oktoberfest: 0 };
...
function nextQuestion() {
    ...
    festivalScores.carnival += selectedOption.scores.carnival;
    festivalScores.tomatina += selectedOption.scores.tomatina;
    festivalScores.oktoberfest += selectedOption.scores.oktoberfest;
...
function showResult() {
    ...
    const festivalData = festivalsData[recommendedFestival];
    
    const resultContainer = document.getElementById('quizResult');
    const maxPossible = quizData.questions.length * 3;
    const list = Object.entries(festivalScores).map(([festival, score]) => {
        const pct = Math.round((score / maxPossible) * 100);
        return `<li>${quizData.results[festival].icon} ${quizData.results[festival].name}: <strong>${pct}%</strong></li>`;
    }).join('');
    ...
    resultContainer.innerHTML = `
        <div class="result-card">
            ...
            <div class="mt-4">
                <small class="text-muted">모든 축제 적합도</small>
